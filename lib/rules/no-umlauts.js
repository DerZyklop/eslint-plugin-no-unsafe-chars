...
module.exports = function(context) {
    ...
    function checkForDisallowedCharactersInString(node, identifier) {
        const unexpectedChar = isNotAllowed(identifier);
        if (typeof identifier !== "undefined" && unexpectedChar) {
            context.report(node, `Unexpected umlaut "${unexpectedChar}" in "${identifier}".`);
        }
    }
